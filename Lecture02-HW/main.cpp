...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // 4. 정점 버퍼 생성 (삼각형 데이터)
    Vertex vertices[] = {
    { posX + 0.0f, posY + 0.5f, 0.5f, 1,0,0,1 },
    { posX + 0.5f, posY - 0.25f, 0.5f, 0,1,0,1 },
    { posX - 0.5f, posY - 0.25f, 0.5f, 0,0,1,1 },
    ...
    D3D11_BUFFER_DESC ibd = { sizeof(indices), D3D11_USAGE_DEFAULT, D3D11_BIND_INDEX_BUFFER, 0, 0, 0 };
    D3D11_SUBRESOURCE_DATA iInitData = { indices, 0, 0 };
    g_pd3dDevice->CreateBuffer(&ibd, &iInitData, &pIBuffer);
    ...
    while (WM_QUIT != msg.message) {
        ...
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
            ...
            if (GetAsyncKeyState(VK_LEFT) & 0x8000)  posX -= 0.001f;
            if (GetAsyncKeyState(VK_RIGHT) & 0x8000) posX += 0.001f;
            if (GetAsyncKeyState(VK_UP) & 0x8000)    posY += 0.001f;
            if (GetAsyncKeyState(VK_DOWN) & 0x8000)  posY -= 0.001f;
            ...
            // 버퍼 새로 생성
            // 루프 안에서는 이름을 다르게
            D3D11_BUFFER_DESC newBd = { sizeof(vertices), D3D11_USAGE_DEFAULT, D3D11_BIND_VERTEX_BUFFER, 0, 0, 0 };
            D3D11_SUBRESOURCE_DATA newInitData = { vertices, 0, 0 };
            g_pd3dDevice->CreateBuffer(&newBd, &newInitData, &pVBuffer);
            ...
            // 최종 그리기
            g_pImmediateContext->IASetIndexBuffer(pIBuffer, DXGI_FORMAT_R32_UINT, 0);

            // 인덱스 기반으로 6개 인덱스 = 삼각형 2개 그리기
